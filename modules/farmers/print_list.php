...
<head>
<meta charset="UTF-8">
<title>FLOUR MILL / FARMER DIRECTORY</title>
<style>
...
<div class="no-print">
    ...
    <a href="list.php" class="btn-back">&#8592; Back to Farmers</a>
</div>

<div class="print-header">
    <h2>FLOUR MILL</h2>
    <h4>FARMER DIRECTORY</h4>
    <div class="date-range">Master List &mdash; All Farmers</div>
</div>

<div class="info-row">
    <span><span class="label">Total Farmers:</span> <?= count($rows) ?></span>
    <span><span class="label">Printed on:</span> <?= date('d-M-Y H:i A') ?></span>
</div>

<?php if (empty($rows)): ?>
    <p style="text-align:center;padding:30px;color:#888;font-size:14px;">No farmers found.</p>
<?php else: ?>
...
<table>
    <thead>
        <tr>
            ...
            <th>Village</th>
            <th>City</th>
            <th class="text-right" width="110">Opening Balance (Rs)</th>
            <th class="text-right" width="110">Current Balance (Rs)</th>
        </tr>
    </thead>
    <tbody>
        ...
        <tr>
            ...
            <td class="text-right"><?= money($row['opening_balance'] ?? 0) ?></td>
            <td class="text-right font-bold <?= ($row['balance'] ?? 0) > 0 ? 'text-danger' : 'text-success' ?>">
                <?= money($row['balance'] ?? 0) ?>
            </td>
        </tr>
        <?php endforeach; ?>
...
<table class="summary-table">
    <tr>
        <td class="font-bold">Total Farmers:</td>
        <td class="text-right"><?= count($rows) ?></td>
    </tr>
    <tr class="grand-row">
        <td>Total Balance:</td>
        <td class="text-right <?= $total_balance > 0 ? 'text-danger' : 'text-success' ?>">Rs <?= money($total_balance) ?></td>
    </tr>
</table>
...
<div class="footer">
    Flour Mill Management System &bull; Farmer Directory &bull; Generated <?= date('d-M-Y H:i:s A') ?>
</div>
