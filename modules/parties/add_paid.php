...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($amount <= 0) { $error = "Amount must be greater than 0."; }
        ...
        try {
            ...
            $paid_notes = ($payment_mode ? ucfirst($payment_mode) : '') . ($notes ? " - $notes" : '');
            $conn->query("INSERT INTO party_transactions (party_id, date, type, amount, balance_after, notes)
                VALUES ($pid, '$date', 'paid', $amount, $new_bal, '$paid_notes')");
            $conn->commit();
            setFlash("Payment of Rs " . number_format($amount, 2) . " to party recorded.");
...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800"><i class="fas fa-money-bill-wave mr-1 text-success"></i> Add Paid</h1>
...
<div class="card shadow mb-4">
    <div class="card-header"><h6 class="font-weight-bold m-0"><i class="fas fa-plus-circle mr-1"></i> Payment Entry <small class="text-muted">(We paid the party)</small></h6></div>
    <div class="card-body">
        <form method="POST">
            <div class="row">
                <div class="col-md-4">
            ...
            <button type="submit" class="btn btn-success"><i class="fas fa-save mr-1"></i> Save Payment</button>
        </form>
    </div>
</div>
